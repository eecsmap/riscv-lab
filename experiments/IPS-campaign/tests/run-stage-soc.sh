...
if [ -f "$CAMP/workloads/build-tlb01.sh" ]; then
  bash "$CAMP/workloads/build-tlb01.sh" "$OUT/b-tlb01" > "$OUT/logs/tlb01-build.log" 2>&1 \
    && cp "$OUT/b-tlb01/tlb01_sfence.elf" "$OUT/elf/" || no "tlb01 built" "see logs/tlb01-build.log"
fi
