...
module tcpu_core #(
  ...
  parameter        TLB_ENTRIES   = 0,   // DISABLED CONFIGURATION: this copy exists only so the
                                       // harness, which does not pass the parameter through, can be
                                       // built against a core with no TLB. Architecturally this must
                                       // be identical to ips-fetch32, and the campaign requires the
                                       // cycle counts to be identical too, not merely close.
  ...
  wire [31:0] fetched_word = pc[2] ? resp_rdata[63:32] : resp_rdata[31:0];
